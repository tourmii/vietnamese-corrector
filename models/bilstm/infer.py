...
class SamplePredictor:
    def __init__(self, src_vocab_path, tgt_vocab_path, model_path, wlm_path,
        ...
        # Patch for PyTorch Adam Optimizer loading issue
        import torch.optim
        _original_setstate = torch.optim.Optimizer.__setstate__
        ...
        self.legal_dict = set()
        try:
            legal_path = "data/legal_vc.txt"
            with open(legal_path, 'r') as f:
                for line in f:
                    vc = line.strip()
                    if vc:
                        self.legal_dict.add(vc)
            ...
            print("Warning: Could not load legal dictionary:", e)
    ...
    def predict_sample(self, sentence):
        line_prep = self.preprocess(sentence)
        
        test = Seq2SeqDataset.from_list([line_prep])
        ...
        with torch.no_grad():
            for batch in test_iterator:
                ...
                # beam search
                predicted_seq = self.beam_lm(''.join(predicted_seq[1:-1]), predicted_values_[1:-1], sentence)

                # match case and punctuations
                predicted_seq = self.match_case(predicted_seq, sentence)
    ...
    def beam_lm(self, predicted_seq, predicted_values, line_raw, k=500, threshold=0.99):
        ...
        def beam_lm_(predicted_seq, predicted_values, predicted_seq_uncertain, k=k, step=0):
            ...
            if step < 10:
                ...
                out = []
                # find disagreements
                # for each disagreement, get top-5 candidates from ltr and rtl search
                for i in range(len(topk_fwd[0])):
                    if topk_fwd[0][i] == topk_bwd[0][i]:
                        out.append(topk_fwd[0][i])
                    else:
...
if __name__ == "__main__":
    ...
    model_path = "checkpoint/bilstm_model"
    wlm_path = "lm/corpus-wplm-4g-v2.binary"

    if len(sys.argv) > 1:
        sample_sentence = sys.argv[1]
    else:
        sample_sentence = "hom nay thoi tiet dep qua, toi muon di choi."

    predictor = SamplePredictor(src_vocab_path, tgt_vocab_path, model_path, wlm_path)
    
    print("Input:", sample_sentence)
    try:
        result = predictor.predict_sample(sample_sentence)
        print("Output:", result)
    except Exception as e:
